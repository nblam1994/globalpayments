...
test("credit auth with convenience", async () => {
    ...
    const report = await src_1.ReportingService.transactionDetail(response.transactionId).execute();
    expect(report).toBeTruthy();
    expect(report.convenienceAmt).toBe("2.00");
});
test.skip("test apple for th web", async () => {
    const card = new src_1.CreditCardData();
    card.mobileType = src_1.MobilePaymentMethodType.APPLEPAY;
    card.paymentSource = src_1.PaymentDataSourceType.APPLEPAYWEB;
...
test.skip("credit auth with cof", async () => {
    const response = await card
        ...
        .withCurrency("USD")
        .withAllowDuplicates(true)
        .withCardBrandStorage(src_1.StoredCredentialInitiator.CardHolder)
        .execute();
    expect(response).toBeTruthy();
    expect(response.responseCode).toBe("00");
    ...
    const cofResponse = await card
        ...
        .withCurrency("USD")
        .withAllowDuplicates(true)
        .withCardBrandStorage(src_1.StoredCredentialInitiator.Merchant, response.cardBrandTransactionId)
        .execute();
...
test("credit verify with cof", async () => {
    const response = await card
        ...
        .withCardBrandStorage(src_1.StoredCredentialInitiator.Merchant)
        .execute();
    expect(response).toBeTruthy();
    expect(response.responseCode).toBe("00");
    expect(response.cardBrandTransactionId).toBeTruthy();
...
test("authorize and capture from transaction", async () => {
    ...
    const authResponse = await card
        ...
        .withAddress(address)
        .execute();
    const captureResponse = await src_1.Transaction.fromId(authResponse.transactionId)
        .capture(10)
        .withCurrency("USD")
        .execute();
...
test("authorize, capture and refund from transaction ID", async () => {
    ...
    const captureResponse = await src_1.Transaction.fromId(authResponse.transactionId)
        ...
        .withCurrency("USD")
        .execute();
    const refundResponse = await src_1.Transaction.fromId(authResponse.transactionId)
        .refund(10)
        .withCurrency("USD")
        .execute();
    expect(authResponse).toBeTruthy();
...
test.skip("credit void from transaction id", async () => {
    ...
    const voidResponse = await src_1.Transaction.fromId(auth.transactionId)
        .void()
        .execute();
...
test("level_iii_response", async () => {
    ...
    const commercialData = new src_1.CommercialData(src_1.TaxType.SalesTax, src_1.TransactionModifier.LevelIII);
...
test("incremental auth", async () => {
    ...
    const origResponse = await card
        ...
        .withAllowDuplicates(true)
        .execute();
    const captureResponse = await src_1.Transaction.fromId(origResponse.transactionId)
        .additionalAuth(12)
        .withModifier(src_1.TransactionModifier.Incremental)
        .withCurrency("USD")
        .execute();
...
test("3D Secure V2", async () => {
    ...
    ecom.xid = "0l35fwh1sys3ojzyxelu4ddhmnu5zfke5vst";
    ecom.eci = 5;
    ecom.version = src_1.Secure3dVersion.TWO;
    card.threeDSecure = ecom;
    const response = await card
        .charge("10.00")
        .withCurrency("USD")
